...
def UserLoginCheck(request):
    if request.method == 'POST':
        ...
        try:
            user = UserProfile.objects.get(email__iexact=email, password=password)
            if user.is_active:
...
def UserHome(request):
    if not is_logged_in(request):
        return redirect('UserLogin')
    total = PredictionHistory.objects.filter(user_id=request.session['user_id']).count()
...
def training(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'training.html', {
            ...
            'best_r2': results[best]['R2'], 'trained': True,
        })
...
def prediction(request):
    ...
    ctx = {'brand_model_map': brand_model_map}
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if year_val < 1990 or year_val > datetime.datetime.now().year:
                errors.append(f'Year must be between 1990 and {datetime.datetime.now().year}.')
        except ValueError:
            errors.append('Year must be a number (e.g. 2019).')

        try:
            km_val = int(form_data['km_driven'])
            if km_val < 500 or km_val > 500000:
                errors.append('KM driven must be between 500 and 5,00,000.')
        except ValueError:
            errors.append('KM driven must be a number (e.g. 45000).')

        if not (MODELS_DIR / 'best_model.pkl').exists():
            errors.append('⚠️ Model not trained yet. Please go to Train Model page first.')
        ...
        input_dict  = {
            ...
            'mileage':           float(form_data['mileage'] or 18),
            'engine':            float(form_data['engine']  or 1200),
            'max_power':         float(form_data['max_power'] or 80),
            'seats':             int(form_data['seats'] or 5),
        }
        ...
        return render(request, 'prediction_result.html', {
            'result': result, 'input': input_dict,
            'similar': similar, 'price_tag': price_tag,
        })
...
def prediction_history(request):
    ...
    stats     = history.aggregate(
        total=Count('id'), avg_price=Avg('predicted_price'),
        max_price=Max('predicted_price'), min_price=Min('predicted_price'),
    )

    return render(request, 'prediction_history.html', {
        'history': history, 'page_obj': page_obj, 'stats': stats,
    })
...
def delete_prediction(request, pk):
    if not is_logged_in(request):
        return redirect('UserLogin')
    try:
        PredictionHistory.objects.get(id=pk, user_id=request.session['user_id']).delete()
...
def compare_cars(request):
    if not is_logged_in(request):
        return redirect('UserLogin')
    return render(request, 'compare.html', {'brand_model_map': brand_model_map})
...
def compare_result(request):
    ...
    def build_car(n):
        ...
        return {
            'year': year, 'vehicle_age': current_year - year,
            'km_driven':         int(request.POST.get(f'km{n}', 0)),
            'fuel_type':         request.POST.get(f'fuel{n}', 'Petrol'),
            'fuel':              request.POST.get(f'fuel{n}', 'Petrol'),
            'seller_type':       request.POST.get(f'seller{n}', 'Individual'),
            'transmission_type': request.POST.get(f'transmission{n}', 'Manual'),
            'transmission':      request.POST.get(f'transmission{n}', 'Manual'),
            'brand':             request.POST.get(f'brand{n}', ''),
            'car_model':         request.POST.get(f'car_model{n}', ''),
            'mileage':           float(request.POST.get(f'mileage{n}') or 18),
            'engine':            float(request.POST.get(f'engine{n}')  or 1200),
            'max_power':         float(request.POST.get(f'power{n}')   or 80),
            'seats':             int(request.POST.get(f'seats{n}')     or 5),
        }
...
def history_detail(request, pk):
    if not is_logged_in(request):
        return redirect('UserLogin')
    try:
        pred = PredictionHistory.objects.get(id=pk, user_id=request.session['user_id'])
    except PredictionHistory.DoesNotExist:
        return redirect('prediction_history')
...
def about(request):
    ...
    if results_path.exists():
        try:
            ...
            best     = max(res, key=lambda x: res[x]['R2'])
            best_r2  = res[best]['R2']
        except Exception:
            pass
